...
pub fn parse_header<'a, T: Reader + ?Sized>(input: &'a T) -> Result<Header<'a>, &'static str> {
    let size_pt1 = mem::size_of::<HeaderPt1>();
    if input.len() < size_pt1 {
        return Err("File is shorter than the first ELF header part");
    }

    let header_1: &'a HeaderPt1 = read(input.read(0, size_pt1));
    ...
    let header_2 = match header_1.class() {
        ...
        Class::ThirtyTwo => {
            ...
            let header_2: &'a HeaderPt2_<P32> =
                read(input.read(size_pt1, mem::size_of::<HeaderPt2_<P32>>()));
        ...
        Class::SixtyFour => {
            ...
            let header_2: &'a HeaderPt2_<P64> =
                read(input.read(size_pt1, mem::size_of::<HeaderPt2_<P64>>()));
...
pub fn sanity_check(file: &ElfFile<'_, impl Reader>) -> Result<(), &'static str> {
